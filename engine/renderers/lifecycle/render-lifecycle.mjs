...
/**
 * Compiles a lifecycle diagram JSON into an SVG string and layout receipt.
 *
 * Brand marks must already be resolved in the `lifecycle` object before
 * calling this function — use `loadDiagramWithBrandMarks` from shared/cli.mjs
 * when loading from disk, or resolve predefined string brand marks yourself.
 * This function performs no I/O (no disk reads, no argv access, no fetch).
 *
 * @param {object} options
 * @param {object} options.lifecycle - Parsed lifecycle diagram JSON (brand marks pre-resolved).
 * @param {string} [options.qualityProfile] - Quality profile override; falls back to lifecycle.meta.quality_profile.
 * @returns {{ ok: boolean, svg?: string, receipt?: object, error?: string, diagnostics?: object[] }}
 */
export function compileLifecycle({ lifecycle, qualityProfile } = {}) {
  const resolvedQualityProfile = qualityProfile ?? lifecycle?.meta?.quality_profile;
  const lifecycleWithProfile = resolvedQualityProfile && !lifecycle?.meta?.quality_profile
    ? { ...lifecycle, meta: { ...lifecycle.meta, quality_profile: resolvedQualityProfile } }
    : lifecycle;

  try {
    return compileLifecycleInternal(lifecycleWithProfile);
  } catch (error) {
...
function compileLifecycleInternal(lifecycle) {
  ...
  function buildLayoutReport() {
    ...
    for (const transition of asArray(lifecycle.transitions)) {
      ...
      const [lx, ly] = labelPoint(transition, pathFor(transition).points);
      const longestLine = Math.max(textUnits(transition.label), textUnits(transition.note || ''));
      const w = Math.max(32, longestLine * 4.9 + 12);
      const h = transition.note ? 27 : 16;
      labels.push({
    ...
    return {
      ...
      transitions: asArray(lifecycle.transitions)
        .filter((t) => states.has(t.from) && states.has(t.to))
        .map((t) => {
          const routed = pathFor(t);
          const labelAt = t.label ? labelPoint(t, routed.points) : null;
          return {
            from: t.from,
            to: t.to,
            label: t.label || null,
            points: routed.points.map(([x, y]) => [Math.round(x), Math.round(y)]),
            ...(labelAt ? { labelAt: [Math.round(labelAt[0]), Math.round(labelAt[1])] } : {}),
          };
        }),
      labels,
    };
...
${renderLegend()}
  ...
  const svg = renderSvg();
  const receipt = buildLayoutReport();
...
if (isMain) {
  ...
  const compiled = compileLifecycle({ lifecycle });

  if (!compiled.ok) {
    const err = new Error(compiled.error || 'Lifecycle compilation failed.');
    err.archifyDiagnostics = compiled.diagnostics ?? [];
    throw err;
  }
