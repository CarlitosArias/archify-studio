...
/**
 * Compiles an architecture diagram JSON into an SVG string and layout receipt.
 *
 * Brand marks must already be resolved in the `architecture` object before
 ...
 * @param {object} options.architecture - Parsed architecture diagram JSON (brand marks pre-resolved).
 * @param {string} [options.qualityProfile] - Quality profile override; falls back to architecture.meta.quality_profile.
 * @returns {{ ok: boolean, svg?: string, receipt?: object, error?: string, diagnostics?: object[] }}
 */
export function compileArchitecture({ architecture: arch, qualityProfile } = {}) {
  const resolvedQualityProfile = qualityProfile ?? arch?.meta?.quality_profile;
  const archWithProfile = resolvedQualityProfile && !arch?.meta?.quality_profile
    ? { ...arch, meta: { ...arch.meta, quality_profile: resolvedQualityProfile } }
    : arch;

  try {
    return compileArchitectureInternal(archWithProfile);
  } catch (error) {
...
function compileArchitectureInternal(arch) {
  ...
  function validateArchitecture() {
    ...
    for (let leftIndex = 0; leftIndex < boundaries.length; leftIndex += 1) {
      ...
      for (let rightIndex = leftIndex + 1; rightIndex < boundaries.length; rightIndex += 1) {
        ...
        const requiresNestedMembership = requiresNestedBoundaryMembership;
        if (!requiresNestedMembership) continue;
...
if (isMain) {
  ...
  const compiled = compileArchitecture({ architecture: arch });

  if (layoutJsonMode) {
    console.log(JSON.stringify(compiled.receipt, null, 2));
    process.exit(compiled.ok ? 0 : 1);
  } else if (!compiled.ok) {
    // Re-throw so the CLI's existing top-level error handler formats it.
    const err = new Error(compiled.error || 'Architecture compilation failed.');
