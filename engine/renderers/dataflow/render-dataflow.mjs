...
/**
 * Compiles a dataflow diagram JSON into an SVG string and layout receipt.
 *
 * Brand marks must already be resolved in the `dataflow` object before calling
 * this function — use `loadDiagramWithBrandMarks` from shared/cli.mjs when
 * loading from disk, or resolve predefined string brand marks yourself.
 * This function performs no I/O (no disk reads, no argv access, no fetch).
 *
 * @param {object} options
 * @param {object} options.dataflow - Parsed dataflow diagram JSON (brand marks pre-resolved).
 * @param {string} [options.qualityProfile] - Quality profile override; falls back to dataflow.meta.quality_profile.
 * @returns {{ ok: boolean, svg?: string, receipt?: object, error?: string, diagnostics?: object[] }}
 */
export function compileDataflow({ dataflow, qualityProfile } = {}) {
  const resolvedQualityProfile = qualityProfile ?? dataflow?.meta?.quality_profile;
  const dataflowWithProfile = resolvedQualityProfile && !dataflow?.meta?.quality_profile
    ? { ...dataflow, meta: { ...dataflow.meta, quality_profile: resolvedQualityProfile } }
    : dataflow;

  try {
    return compileDataflowInternal(dataflowWithProfile);
  } catch (error) {
...
function compileDataflowInternal(dataflow) {
  ...
  function buildLayoutReport() {
    return {
      ...
      viewBox,
      stages: compositionFrames.map((frame) => ({
        id: frame.id,
        label: frame.label,
        x: frame.x,
        y: frame.y,
      ...
      flows: asArray(dataflow.flows)
        .filter((flow) => nodes.has(flow.from) && nodes.has(flow.to))
        .map((flow) => {
          const routed = pathFor(flow);
          const labelAt = flow.label ? labelPoint(flow, routed.points) : null;
          return {
            from: flow.from,
            to: flow.to,
            label: flow.label || null,
            points: routed.points.map(([x, y]) => [Math.round(x * 10) / 10, Math.round(y * 10) / 10]),
...
if (isMain) {
  ...
  const compiled = compileDataflow({ dataflow });

  if (!compiled.ok) {
    const err = new Error(compiled.error || 'Dataflow compilation failed.');
