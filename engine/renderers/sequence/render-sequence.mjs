...
/**
 * Compiles a sequence diagram JSON into an SVG string and layout receipt.
 *
 * Brand marks must already be resolved in the `sequence` object before calling
 * this function — use `loadDiagramWithBrandMarks` from shared/cli.mjs when
 * loading from disk. This function performs no I/O (no disk reads, no argv
 * access, no fetch).
 *
 * @param {object} options
 * @param {object} options.sequence - Parsed sequence diagram JSON (brand marks pre-resolved).
 * @param {string} [options.qualityProfile] - Quality profile override; falls back to sequence.meta.quality_profile.
 * @returns {{ ok: boolean, svg?: string, receipt?: object, error?: string, diagnostics?: object[] }}
 */
export function compileSequence({ sequence, qualityProfile } = {}) {
  const resolvedQualityProfile = qualityProfile ?? sequence?.meta?.quality_profile;
  const sequenceWithProfile = resolvedQualityProfile && !sequence?.meta?.quality_profile
    ? { ...sequence, meta: { ...sequence.meta, quality_profile: resolvedQualityProfile } }
    : sequence;

  try {
    return compileSequenceInternal(sequenceWithProfile);
  } catch (error) {
...
function compileSequenceInternal(sequence) {
  const viewBox = sequence.meta?.viewBox || [920, 760];
  // `column_fit: "spread"` widens lanes with the viewBox instead of keeping the
  // fixed 108px gap, so a wide canvas gains column distance and label room
  ...
  function buildLayoutReport() {
    return {
      ...
      layout: {
        ...
        participantW,
        colGap,
        lifelineTop: layout.lifelineTop,
      ...
      messages: asArray(sequence.messages)
        .filter((m) => participants.has(m.from) && participants.has(m.to) && typeof m.y === 'number')
        .map((m) => {
          const { start, end, center } = messageGeometry(m);
          return {
            label: m.label,
            from: m.from,
            to: m.to,
      ...
      segments: asArray(sequence.segments).map((s) => ({
        label: s.label,
        from: s.from,
        to: s.to,
      })),
      activations: asArray(sequence.activations).map((a) => ({
...
if (isMain) {
  ...
  const compiled = compileSequence({ sequence });

  if (!compiled.ok) {
    const err = new Error(compiled.error || 'Sequence compilation failed.');
